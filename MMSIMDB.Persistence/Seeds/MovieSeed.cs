...
namespace MMSIMDB.Persistence.Seeds
{
    public class MovieSeed
    {
        ...
        {
            if (!movieRepositoryAsync.GetAll().Any(el => el.MovieTypeID == (int)MovieTypeEnum.Movie))
            {
                string fileLocation = System.IO.Path.Combine(AppContext.BaseDirectory, @"Seeds\json\movie.json");
